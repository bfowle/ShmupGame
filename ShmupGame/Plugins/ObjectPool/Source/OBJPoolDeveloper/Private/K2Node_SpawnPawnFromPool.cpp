...
void UK2Node_SpawnPawnFromPool::AllocateDefaultPins() {
	...
	FEdGraphPinType PINT_Owner(K2Schema->PC_Object,TEXT(""),APawn::StaticClass(),EPinContainerType::None,false,FEdGraphTerminalType());
	...
	FEdGraphPinType PINT_Result(K2Schema->PC_Object,TEXT(""),APooledPawn::StaticClass(),EPinContainerType::None,false,FEdGraphTerminalType());
	...
	if (GetBlueprint()->ParentClass->HasMetaDataHierarchical(FBlueprintMetadata::MD_ShowWorldContextPin)) {
		CreatePin(EGPD_Input,PINT_Object,FK2Node_SpawnPawnFromPoolHelper::WorldContextPinName);
	}///
	//
	/// Object-Pool Pin.
	UEdGraphPin* PIN_Pool = CreatePin(EGPD_Input,PINT_Object,FK2Node_SpawnPawnFromPoolHelper::PoolPinName);
	//
	/// Owner Pin.
	UEdGraphPin* PIN_Owner = CreatePin(EGPD_Input,PINT_Owner,FK2Node_SpawnPawnFromPoolHelper::OwnerPinName);
	PIN_Owner->bAdvancedView = true;
	//
	/// Collision Handling Pin.
	UEdGraphPin* const PIN_CollisionHandlingOverride = CreatePin(EGPD_Input,PINT_CHO_Enum,FK2Node_SpawnPawnFromPoolHelper::CollisionHandlingOverridePinName);
	...
	//
	/// Reconstruct Pin.
	UEdGraphPin* PIN_Transform = CreatePin(EGPD_Input,PINT_Transform,FK2Node_SpawnPawnFromPoolHelper::SpawnTransformPinName);
	UEdGraphPin* PIN_Options = CreatePin(EGPD_Input,PINT_SpawnOptions,FK2Node_SpawnPawnFromPoolHelper::SpawnOptionsPinName);
	UEdGraphPin* PIN_Reconstruct = CreatePin(EGPD_Input,PINT_Bool,FK2Node_SpawnPawnFromPoolHelper::ReconstructPinName);
	UEdGraphPin* PIN_Success = CreatePin(EGPD_Output,PINT_Bool,FK2Node_SpawnPawnFromPoolHelper::SuccessPinName);
		...
		if (Pin->PinName == FK2Node_SpawnPawnFromPoolHelper::NoCollisionFailPinName) {
			...
			if (Pin->LinkedTo.Num() == 0) {
				...
				if (CollisionHandlingOverridePin) {
					UEnum const* const MethodEnum = FindObjectChecked<UEnum>(ANY_PACKAGE, TEXT("ESpawnPawnCollisionHandlingMethod"), true);
				...
				//
				UEnum* const MethodEnum = FindObjectChecked<UEnum>(ANY_PACKAGE, TEXT("ESpawnPawnCollisionHandlingMethod"), true);
				//
